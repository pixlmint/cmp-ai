...
# Trigger tokens for syntax-aware incomplete-line spans
_TRIGGER_TOKENS = [
...
def generate_incomplete_line_spans(
    ...
    for _ in range(target_count // 2 + 1):
        # Pick a non-empty, non-comment line
        candidates = [
    ...
    for _ in range(target_count // 2 + 1):
        # Find lines containing trigger tokens
        trigger_candidates = []
        ...
        # Cut right after the trigger token
        cut_in_line = indent_len + tok_pos_in_stripped + len(tok)
        line_start = sum(len(l) + 1 for l in lines[:line_idx])

        # If tree-sitter available, extend to end of statement node
        end_byte = line_start + len(line)
        if tree:
            cut_byte_abs = line_start + cut_in_line
            # Find statement containing this position
...
def generate_bracket_context_spans(
    ...
    # Node types that represent bracket-delimited content
    bracket_types = {
    ...
    def _collect_bracket_nodes(node: "Node"):
        if node.type in bracket_types and node.child_count >= 2:
            # Must have opening and closing delimiters with content between
    ...
    for node in chosen:
        # Middle = content between first and last child (the delimiters)
    ...
    def _collect_comment_spans(node: "Node"):
        for i, child in enumerate(node.children):
            if child.type == "comment":
                # Check this comment occupies a full line
            ...
            # Recurse into compound statements etc.
