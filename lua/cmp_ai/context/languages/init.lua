...
function M.get_handler(filetype)
  ...
  if not handler_name then
    ...
    local status, handler = pcall(require, 'cmp_ai.context_providers.languages.' .. filetype)
    if status then
    ...
    handler = require('cmp_ai.context_providers.languages.base')
    handlers[filetype] = handler
    return handler
  end

  -- Load the mapped handler
  local status, handler = pcall(require, 'cmp_ai.context_providers.languages.' .. handler_name)
  if status then
    handlers[filetype] = handler
    return handler
  end
...
function M.has_handler(filetype)
  return filetype_map[filetype] ~= nil or
      pcall(require, 'cmp_ai.context_providers.languages.' .. filetype)
end
